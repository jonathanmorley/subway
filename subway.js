...
(function() {
  ...
  drawRoute = function(paper, route) {
    ...
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      edge = _ref[_i];
      _results.push(paper.path(Raphael.fullfill("M{start.x},{start.y}L{end.x},{end.y}", edge)).transform(route.translate));
    }
    return _results;
  };
  ...
  getStartEndPoint = function(edges) {
    ...
    for (_i = 0, _len = edges.length; _i < _len; _i++) {
      ...
      _results.push({
        start: {
          x: edge.follow ? current.x = _results[edge.follow].end.x : current.x,
          y: edge.follow ? current.y = _results[edge.follow].end.y : current.y,
          curve: __indexOf.call(edges.slice(1), edge) >= 0
        },
        end: {
          ...
          curve: __indexOf.call(edges.slice(0), edge) >= 0 || edges[_i + 1].follow
        }
      });
    }
    return _results;
