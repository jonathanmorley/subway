...
(function() {
  var drawGridLines, drawHorizontalLabel, drawHorizontalLine, drawRoute, drawSolidEdge, drawStation, drawVerticalLabel, drawVerticalLine, getStartEndPoint, map,
  ...
  drawGridLines = function(paper) {
    var cell_length, clearbox, i, x, _i, _j, _k, _l, _m, _n, _o, _p, _ref, _ref1, _ref2, _ref3, _ref4, _ref5, _ref6, _ref7;
    cell_length = Math.min(map.width / map.grid.columns, map.height / map.grid.rows);
    for (i = _i = 1, _ref = map.grid.rows; 1 <= _ref ? _i < _ref : _i > _ref; i = 1 <= _ref ? ++_i : --_i) {
      drawHorizontalLine(paper, map.border + i * cell_length, 0.25, cell_length);
    }
    for (i = _j = 1, _ref1 = map.grid.columns; 1 <= _ref1 ? _j < _ref1 : _j > _ref1; i = 1 <= _ref1 ? ++_j : --_j) {
      drawVerticalLine(paper, map.border + i * cell_length, 0.25, cell_length);
    }
    clearbox = {
      attr: {
        stroke: "white",
    ...
    for (i = _k = 0, _ref2 = map.grid.columns; 0 <= _ref2 ? _k <= _ref2 : _k >= _ref2; i = 0 <= _ref2 ? ++_k : --_k) {
      drawHorizontalLabel(paper, map.border + (i - 0.5) * cell_length, clearbox);
    }
    for (i = _l = 0, _ref3 = map.grid.rows; 0 <= _ref3 ? _l <= _ref3 : _l >= _ref3; i = 0 <= _ref3 ? ++_l : --_l) {
      drawVerticalLabel(paper, map.border + (i - 0.5) * cell_length, clearbox);
    }
    paper.setStart();
    ...
    for (x = _m = 0, _ref4 = map.grid.columns; 0 <= _ref4 ? _m <= _ref4 : _m >= _ref4; x = 0 <= _ref4 ? ++_m : --_m) {
    ...
    for (x = _n = 1, _ref5 = map.grid.columns; 1 <= _ref5 ? _n <= _ref5 : _n >= _ref5; x = 1 <= _ref5 ? ++_n : --_n) {
    ...
    for (x = _o = 0, _ref6 = map.grid.rows; 0 <= _ref6 ? _o <= _ref6 : _o >= _ref6; x = 0 <= _ref6 ? ++_o : --_o) {
    ...
    for (x = _p = 1, _ref7 = map.grid.rows; 1 <= _ref7 ? _p <= _ref7 : _p >= _ref7; x = 1 <= _ref7 ? ++_p : --_p) {
  ...
  drawHorizontalLine = function(paper, position, width, cell_length) {
    return paper.path(Raphael.format("M{0},{1}l{2},0", map.border, position, cell_length * map.grid.columns)).attr("stroke", map.grid.color).attr("stroke-width", map.multiplier * width);
  };

  drawVerticalLine = function(paper, position, width, cell_length) {
    return paper.path(Raphael.format("M{0},{1}l0,{2}", position, map.border, cell_length * map.grid.rows)).attr("stroke", map.grid.color).attr("stroke-width", map.multiplier * width);
  };

  drawHorizontalLabel = function(paper, position, clearbox) {
    paper.rect(position - clearbox.width / 2, map.border - clearbox.height / 2, clearbox.width, clearbox.height).attr(clearbox.attr);
    return paper.text(position, map.border, x).attr("fill", map.grid.color).attr("font-size", map.multiplier * 13);
  };

  drawVerticalLabel = function(paper, position, clearbox) {
    paper.rect(map.border - clearbox.width / 2, position - clearbox.height / 2, clearbox.width, clearbox.height).attr(clearbox.attr);
    return paper.text(map.border, position, String.fromCharCode(x + 64)).attr("fill", map.grid.color).attr("font-size", map.multiplier * 13);
  };
